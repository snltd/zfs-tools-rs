...
fn restore_action(file: &Utf8Path, cli: &Cli) -> anyhow::Result<CopyAction> {
...
fn backup_target(src: &Utf8Path, cli: &Cli) -> anyhow::Result<()> {
    ...
    if cli.noop {
        ...
        Err(anyhow!("Backup target {} exists", dest))
    } else {
        fs::rename(src, dest)?;
        Ok(())
    }
...
fn candidates(filesystem_root: &Utf8Path, file: &Utf8Path, cli: &Cli) -> io::Result<Candidates> {
    let snapshot_dirs = match all_snapshot_dirs(filesystem_root) {
        Some(dirs) => dirs,
        None => {
            eprintln!("No snapshots found under {filesystem_root}");
            return Ok(Vec::new());
        }
    };
    ...
    let relative_path = match path_relative_to_fs_root(file, filesystem_root) {
        Some(path) => path,
        None => {
            eprintln!("Failed to calculate path for {file} relative to {filesystem_root}");
            return Ok(Vec::new());
        }
    };
        ...
        .filter_map(|snapdir| {
            ...
            if candidate.exists() {
                ...
                match details_of(snapdir, &candidate) {
                    ...
                    Err(e) => {
                        eprintln!("Failed to get mtime for {candidate}: {e}");
                        None
                    }
                }
            } else {
...
fn details_of(snapdir: &Utf8Path, file: &Utf8Path) -> io::Result<Candidate> {
    let metadata = fs::metadata(file)?;

    let candidate = Candidate {
        snapname: snapdir.file_name().unwrap().to_string(),
        path: file.to_owned(),
...
fn path_relative_to_fs_root(file: &Utf8Path, filesystem_root: &Utf8Path) -> Option<Utf8PathBuf> {
    file.strip_prefix(filesystem_root)
        .ok()
        .map(|p| p.to_owned())
}
...
fn main() {
    ...
    for file in &cli.file_list {
        let f = match canonical_file(Utf8PathBuf::from(file)) {
            Ok(file) => file,
            Err(e) => {
        ...
        match restore_action(&Utf8PathBuf::from(&f), &cli) {
            Ok(Some((src, dest))) => {
                if let Err(e) = file_copier::copy_file(&src, &dest, &opts) {
                    eprintln!("ERROR restoring {f}: {e}");
                    errs += 1;
                }
            }
            Ok(None) => (),
            Err(e) => {
                eprintln!("ERROR working out how to restore {f}: {e}");
                errs += 1;
            }
        }
    }
...
mod test {
    ...
    fn test_candidates() {
        ...
        assert!(
            candidates(&fixture(""), &fixture("file_in_neither"), &cli)
                .unwrap()
                .is_empty()
        );
    }
    ...
    fn test_restore_action_auto_mode() {
        ...
        let cli = Cli {
            file_list: vec![file_path.to_string()],
            verbose: false,
            noop: false,
            auto: true,
            noclobber: false,
